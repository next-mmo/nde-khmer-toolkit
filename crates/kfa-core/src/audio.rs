//! Audio loading. WAV → mono 16 kHz f32.
//!
//! The Python version uses librosa; we use `hound` for WAV decoding plus a
//! simple linear resampler. Input WAVs that are already 16 kHz mono (the
//! common case for kfa use) bypass resampling.
...
/// Load a WAV file, downmix to mono, resample to 16 kHz, and return samples in [-1, 1].
pub fn load_wav_mono_16k(path: impl AsRef<Path>) -> Result<Vec<f32>> {
