//! CTC forced-alignment helper functions.
//!
//! Ported from Python `kfa/utils.py`. Uses `ndarray` in place of numpy.

use ndarray::{Array2, ArrayView2};

pub const STRIDE_MSEC: f64 = 20.0;
...
///
/// Mirrors the Python `intersperse` helper: the resulting vec has length
/// `2 * lst.len() + 1`, starting and ending with `item`.
...
/// Convert a time in seconds to a model frame index.
...
///
/// `emission` is `[num_frames, vocab_size]`; `tokens` are token ids.
pub fn get_trellis(emission: ArrayView2<f64>, tokens: &[usize], blank_id: usize) -> Array2<f64> {
    ...
    // cumulative sum of emission[1..,blank] into trellis[1..,0]
...
pub fn merge_repeats(path: &[Point], transcript: &[char]) -> Vec<Segment> {
    ...
    while i1 < path.len() {
        ...
        let score: f64 =
            (i1..i2).map(|k| path[k].score).sum::<f64>() / (i2 - i1) as f64;
        let label = transcript[path[i1].token_index].to_string();
...
/// Apply log-softmax along the last axis of a 2-D array, in place.
...
mod tests {
    ...
    fn test_time_to_frame() {
        ...
        assert_eq!(time_to_frame(1.0), 50);
        assert_eq!(time_to_frame(0.02), 1);
    }
}
